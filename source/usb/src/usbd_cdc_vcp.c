...
// changed by anve for usb
  ...
  for(i=0; i<Len; i++)
  {
  	//if(((APP_Rx_ptr_in+1) & (APP_RX_DATA_SIZE-1)) == APP_Rx_ptr_out)
    //{
  	  // buffer is full
  	//  return USBD_FAIL;
    //}
...
static uint16_t VCP_DataRx (uint8_t* Buf, uint32_t Len)
{
  ...
  for(i=0; i<Len; i++)
  {
  	IsrReceiveCommand(COMMAND_FROM_USB, Buf[i]);
  }
#else
  uint32_t i;
...
void USB_TO_USART_DATA(uint8_t *buf, int len)
{
	
}

void USART_TO_USB_DATA(uint8_t *buf, int len)
{
	VCP_DataTx (buf,len);
}
void USB_COM_IRQHandler(void)
{
