...
SD_Error SD_ReadDisk(uint8_t * buf, uint32_t sector,	uint8_t cnt)
{
	...
	if(cnt == 1)
	{
		error = SD_ReadBlock(sector*SD_BLOCK_SIZE,(uint32_t*)buf, SD_BLOCK_SIZE);
	}
	else
	{
		error = SD_ReadMultiBlocks(sector*SD_BLOCK_SIZE,(uint32_t*)buf, SD_BLOCK_SIZE, cnt);
	}
	
	if(error != SD_OK)
	{
		printf("SD_ReadDisk Error=%d\r\n", error);
	}
...
SD_Error SD_WriteDisk(const uint8_t * buf, uint32_t sector,uint8_t cnt)
{
	SD_Error error = SD_OK;
	//printf("SD_WriteDisk sector=%d, cnt=%d\r\n", sector, cnt);
	if((int)buf & 0x03)
	{
	...
	if(cnt == 1)
	{
		error = SD_WriteBlock(sector*SD_BLOCK_SIZE,(uint32_t*)buf, SD_BLOCK_SIZE);
	}
	else
	{
		error = SD_WriteMultiBlocks(sector*SD_BLOCK_SIZE,(uint32_t*)buf, SD_BLOCK_SIZE, cnt);
	}
	//printf("SD_WriteDisk End\r\n");
	if(error != SD_OK)
	{
		printf("SD_WriteDisk Error=%d\r\n", error);
	}
	return error;
}
...
static void DMA_TxConfiguration(uint32_t *BufferSRC, uint32_t BufferSize)
{
  ...
  DMA_ClearFlag(DMA2_Stream4,DMA_FLAG_TCIF4| DMA_FLAG_TEIF4| DMA_FLAG_HTIF4| DMA_FLAG_DMEIF4);
