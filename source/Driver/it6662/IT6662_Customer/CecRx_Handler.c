...
void CecRx_PushCmd2CecTx(iTE_u8 ucFollower, eCEC_CMD TxCmd, iTE_u8 u8CecSize)
{
	...
	Cec_TxCmdPush(ucFollower, TxCmd, u8CecSize);
...
void CecRx_OtpFeature(void)
{
	switch(g_stCecCur->stCecRxCmd.ucOpcode){
		case	eImageViewOn:
		case	eTextViewOn:
				CecRx_PushCmd2CecTx(DEVICE_ID_TV, g_stCecCur->stCecRxCmd.ucOpcode, 2);
			break;
		case	eActiveSource:
				CecRx_PushCmd2CecTx(DEVICE_ID_TV, g_stCecCur->stCecRxCmd.ucOpcode, 4);
				iTE_Cec_Msg(("[%02X%02X]\n", g_stCecCur->stCecRxCmd.ucOperand1, g_stCecCur->stCecRxCmd.ucOperand2));
	...
	switch(g_stCecCur->stCecRxCmd.ucOpcode){
		...
		case	eInactiveSource:
				if(g_stCecCur->u8MyLogAdr == DEVICE_ID_TV){
					// TV may display its internal tuner & send an <Active Source>
					// or , TV should send <Set Stream Path > to another device for display
					...
					if(0){
						...
						g_stCecCur->stCecTxCmd.ucOperand1 = g_stCecCur->u8PaL;
						g_stCecCur->stCecTxCmd.ucOperand2 = g_stCecCur->u8PaH;
						Cec_TxCmdPush(DEVICE_ID_TV , eInactiveSource, 4);
						Cec_BlockSel(CEC_RX_SEL);
					}
